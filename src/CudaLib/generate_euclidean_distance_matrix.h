...
void generate_euclidean_distance_matrix(thrust::device_vector<T>& d_euclideanDistanceMatrix,
    thrust::device_vector<uint32_t>& d_bestRotationMatrix, uint32_t som_size, thrust::device_vector<T> const& d_som,
	uint32_t image_size, uint32_t num_rot, thrust::device_vector<T> const& d_rotatedImages, uint32_t block_size,
...
//            d_firstStep, som_size, num_rot, image_size, block_size);
//    } else {
