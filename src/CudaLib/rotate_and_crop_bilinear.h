...
template <typename T>
__global__ void
rotate_and_crop_bilinear(thrust::device_ptr<T> rotatedImages, thrust::device_ptr<T> image, int neuron_size,
    int neuron_dim, int image_dim, thrust::device_ptr<T> const& cosAlpha,
    thrust::device_ptr<T> const& sinAlpha, int numberOfChannels)
{
